...
<div class="content">
    <div class="local">
        <span>当前位置：<a href="app_index/goindex">普宁档案</a>>><a href="app_index/goArchiveIntroduction">局馆介绍</a><span>>>局馆介绍</span></span>
    </div>
    <div class="details">
        <div class="navBox">
            <h2>局馆介绍</h2>
            <ul class="nav_option">
                   <li><a href="app_index/goArchiveIntroduction" class="active">局馆介绍</a></li>
...
<script type="text/javascript">
  ...
  $().ready(function(){
      $.ajax({
			type: "POST",
			url: '<%=basePath%>app_doc/getDocList.do?type=14&tm='+new Date().getTime()+'&currentPage=1&showCount=15',
			...
			success : function(data) {
				...
				$(".paging").empty();
				$(".paging").append("<input  type='button' id='14"+"' value='首页' onclick='switchPage(1)'>");
				$(".paging").append("<input type='button' id='14"+"' value='上页' onclick='switchPage("+previousPage+")'>");
				$(".paging").append("<font color='red'>"+data.currentPage+"</font>"
						+"/"+data.totalPage);
				$(".paging").append("<input type='button' id='14"+"' value='下页' onclick='switchPage("+nextPage+")'>");
				$(".paging").append("<input type='button' id='14"+"' value='尾页' onclick='switchPage("+data.totalPage+")'>");
			}
		});
  });
    ...
    function switchPage(page){
   	 $.ajax({
			type: "POST",
			url: '<%=basePath%>app_doc/getDocList.do?type=14'+'&tm='+new Date().getTime()+'&currentPage='+page+'&showCount=15',
	    	data: {},
			dataType:'json',
			cache: false,
			success: function(data){
