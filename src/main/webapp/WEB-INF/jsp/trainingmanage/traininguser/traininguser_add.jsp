					...
					<div class="row">
						...
						<form action="traininguser/${msg }.do" name="userForm" id="userForm" method="post">
							...
							<table id="table_report" class="table table-striped table-bordered table-hover">
								...
								<input hidden="hidden" type="text" name="BZ" id="BZ"value="${pd.BZ }" placeholder="这里输入备注" title="备注" style="width:98%;" />
								<input hidden="hidden" type="text" name="NUMBER" id="NUMBER" value="${pd.NUMBER }" maxlength="32" placeholder="这里输入编号" title="编号" onblur="hasN('${pd.USERNAME }')" style="width:98%;" />
								<input hidden="hidden" class="span10 date-picker" name="START_TIME" id="START_TIME" value="${pd.START_TIME}" type="text" data-date-format="yyyy-mm-dd" readonly="readonly" placeholder="开通日期"  title="开通日期" style="width:98%;" />
								<input hidden="hidden" type="role" name="ROLE" id="ROLE"value="${pd.ROLE_ID }" placeholder="角色" title="角色" style="width:98%;" />
...
<script type="text/javascript">
	...
	function firstAdd(){
			$.ajax({
				url:'<%=basePath%>traininguser/firstAdd.do',
				success:function(data){
					...
					$("#chkpwd").val(data.PASSWORD);
					$("#END_TIME").val(data.END_TIME);
					$("#YEARS").val(data.YEARS);
				}
			})
		}
	window.onload = firstAdd;
