			...
			<div class="main-content-inner">
				<div class="page-content">
					<div class="row">
						<div class="col-xs-12">
						<!-- 检索  -->
						<form action="traininguser/list.do" method="post" name="userForm" id="userForm">
										...
										<tr>
											...
											<td class="center">
												...
												<div class="hidden-sm hidden-xs btn-group">
													<c:if test="${QX.cha == 1 }">
													<a class="btn btn-xs btn-success" onclick="details('${user.USER_ID}');">
	...
	<script type="text/javascript" src="static/js/jquery.tips.js"></script>
 	...
 	<script type="text/javascript">
		...
		function add(){
			 ...
			 diag.Drag=true;
			 diag.Title ="新增学员";
			 diag.URL = '<%=basePath%>traininguser/goAdd.do';
			 diag.Width = 600;
			 diag.Height = 419;
			 diag.CancelEvent = function(){ //关闭事件
		...
		function editUser(user_id){
			 ...
			 diag.Drag=true;
			 diag.Title ="会员资料";
			 diag.URL = '<%=basePath%>traininguser/goEdit.do?USER_ID='+user_id;
			 diag.Width = 600;
			 diag.Height = 419;
			 diag.CancelEvent = function(){ //关闭事件
				 if(diag.innerFrame.contentWindow.document.getElementById('zhongxin').style.display == 'none'){
		...
		function delUser(userId,msg){
			bootbox.confirm("确定要删除["+msg+"]吗?", function(result) {
				if(result) {
					top.jzts();
					var url = "<%=basePath%>traininguser/delete.do?USER_ID="+userId+"&tm="+new Date().getTime();
				...
				if(result) {
					...
					if(str==''){
						...
						if(msg == '确定要删除选中的数据吗?'){
							...
							$.ajax({
								type: "POST",
								url: '<%=basePath%>traininguser/deleteAll.do?tm='+new Date().getTime(),
						    	data: {USER_IDS:str},
								dataType:'json',
		...
		function details(Id){
			 ...
			 diag.Drag=true;
			 diag.Title ="细阅";
			 diag.URL = '<%=basePath%>traininguser/goDetails.do?USER_ID='+Id;
			 diag.Width = 600;
			 diag.Height = 419;
			 diag.CancelEvent = function(){ //关闭事件
				 if(diag.innerFrame.contentWindow.document.getElementById('zhongxin').style.display == 'none'){
