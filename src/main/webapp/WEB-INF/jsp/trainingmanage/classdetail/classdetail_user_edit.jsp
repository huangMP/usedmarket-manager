								...
								<c:when test="${not empty varList}">
									...
									<c:forEach items="${varList}" var="var" varStatus="vs">
											<tr>
												...
												<td class='center'>
													<c:if test="${QX.edit == 1 }">
														<a class="btn btn-xs btn-danger" onclick="delUser('${pd.CLASSDETAIL_ID}','${var.USER_ID}');">
						...
						<table style="width:100%;">
							<tr>
								<td style="vertical-align:top;">
									<c:if test="${QX.del == 1 }">
									<a class="btn btn-sm btn-danger" onclick="delUserAll('${pd.CLASSDETAIL_ID}');">移除学员</a>
									</c:if>
...
<!-- /.main-container -->
	...
	<script type="text/javascript" src="static/js/jquery.tips.js"></script>
		...
		function delUser(clsId,uId){
			
			$.ajax({
				url:'<%=basePath%>classdetail/delUser.do?CLASSDETAIL_ID='+clsId+'&USER_ID='+uId,
		...
		function delUserAll(clsId){
			...
			if(str==''){
				...
				$.ajax({
					type: "POST",
					url: '<%=basePath%>classdetail/delUserAll.do?CLASSDETAIL_ID='+clsId+'&tm='+new Date().getTime(),
			    	data: {DATA_IDS:str},
					dataType:'json',
