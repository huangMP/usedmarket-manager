								...
								<c:when test="${not empty varList}">
									...
									<c:forEach items="${varList}" var="var" varStatus="vs">
											<tr>
												...
												<td class='center'>
													<c:if test="${QX.edit == 1 }">
														<a class="btn btn-xs btn-success" onclick="addSpecial('${pd.CLASSDETAIL_ID}','${var.SPECIAL_ID}');">
						...
						<table style="width:100%;">
							<tr>
								<td style="vertical-align:top;">
									<c:if test="${QX.add == 1 }">
									<a class="btn btn-sm btn-success" onclick="addSpecialAll('${pd.CLASSDETAIL_ID}');">添加专题</a>
									</c:if>
...
<!-- /.main-container -->
	...
	<script type="text/javascript" src="static/js/jquery.tips.js"></script>
		...
		function addSpecial(clsId,spId){
			
			$.ajax({
				url:'<%=basePath%>classdetail/addSpecial.do?CLASSDETAIL_ID='+clsId+'&SPECIAL_ID='+spId,
		...
		function addSpecialAll(clsId){
			...
			if(str==''){
				...
				$.ajax({
					type: "POST",
					url: '<%=basePath%>classdetail/addSpecialAll.do?CLASSDETAIL_ID='+clsId+'&tm='+new Date().getTime(),
			    	data: {DATA_IDS:str},
					dataType:'json',
